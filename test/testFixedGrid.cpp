...
TEST_F(TestOptimalLayoutFixture, TestPartitioningFixedGrid){
    ...
    ASSERT_EQ(readColumn<arrow::StringArray>(pathPartition1, "city"), std::vector<std::string>({"Atlanta", "Miami"}));
    ASSERT_EQ(readColumn<arrow::Int32Array>(pathPartition1, "x"), std::vector<int32_t>({85, 90}));
    ASSERT_EQ(readColumn<arrow::Int32Array>(pathPartition1, "y"), std::vector<int32_t>({15, 5}));
    auto pathPartition2 = folder / ("2" + fileExtension);
    ASSERT_EQ(readColumn<arrow::StringArray>(pathPartition2, "city"), std::vector<std::string>({"Denver", "Chicago"}));
    ASSERT_EQ(readColumn<arrow::Int32Array>(pathPartition2, "x"), std::vector<int32_t>({5, 35}));
    ASSERT_EQ(readColumn<arrow::Int32Array>(pathPartition2, "y"), std::vector<int32_t>({45, 42}));
    ...
    ASSERT_EQ(readColumn<arrow::Int32Array>(pathPartition0, "partition_id"), std::vector<int32_t>({0, 0, 0}));
    ...
    ASSERT_EQ(readColumn<arrow::Int32Array>(pathPartition1, "partition_id"), std::vector<int32_t>({1, 1}));
}
